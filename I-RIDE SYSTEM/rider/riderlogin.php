...
$riders_table_sql = "CREATE TABLE IF NOT EXISTS Riders (
    rider_id INT PRIMARY KEY AUTO_INCREMENT,
    full_name VARCHAR(100) NOT NULL,
    phone_number VARCHAR(20) NOT NULL,
    email VARCHAR(100),
    password_hash VARCHAR(255) NOT NULL,
    booking_type ENUM('same_day_hatid', 'same_day_delivery', 'pet_transport') NOT NULL,
...
if ($conn->query($riders_table_sql) === FALSE) {
    die("Error creating Riders table: " . $conn->error);
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($result) == 1) {
        ...
        if (password_verify($password, $row['password'])) {
            ...
            $_SESSION['rider_id'] = $row['rider_id']; // Assuming UserID is the primary key of the users table
            $_SESSION['username'] = $row['username'];

            // Redirect to rider.php after successful login
            header("Location: RDBookingtest.php");
            exit();
        } else {
            // Password is incorrect
            $_SESSION['login_error'] = "Invalid username or password";
            header("Location: riderlogin.php"); // Redirect back to login page with error message
            exit();
        }
    } else {
        // Username not found
